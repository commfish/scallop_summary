<<set-parent_ksw, echo=FALSE,cache=FALSE,eval=TRUE>>=
set_parent('Parent.Rnw')
@

<<load_ksw,echo=FALSE,message=FALSE,warning=FALSE>>=
library(tidyverse)
library(mgcv)
library(maps)
library(mapdata)
...
<<data_ksw,echo=FALSE,message=FALSE,warning=FALSE>>=
...
x %>% 
	group_by(FY) %>% 
	summarise(m=sum(meat_weight), r = sum(round_weight), h = sum(dredge_hrs), n=length(dredge_hrs), c1 = m/h, c2 = r/h) -> kswt1
mghl = data.frame(ghl=rep(25000,7))

kswt1 <- bind_cols(mghl,kswt1) %>% 
...
\textbf{Biometric synopsis}: CPUE is up relative to the past few years. There appears to be a new recruitment coming on -many small sized shells may account for the increase in discards.

  The 2016/17 Kodiak Southwest District scallop fishery opened on July 1, 2016 with a GHL of 25,000 lb of scallop meat. One vessel particpated in the fishery (Figure \ref{fig:kswvessel}) harvesting 25,120 lb scallop meat with a cpue of 47 lb meat/dredge hour (Table \ref{table:kswt1}). No distinct trends in size of scallops are observed (scallops are not yet aged from the 2016/17 season).
   ...
   \label{table:kswt1}
    	\begin{tabular}{lcrrcccc}
  		...
  		\hline	
			<<kswidtbl,echo=FALSE,message=FALSE,warning=FALSE, results='asis'>>=		
			print(xtable(kswt1,align="llcrrcccc",digits=0),only.contents=TRUE, include.rownames=FALSE, 
...
<<model_ksw,echo=FALSE,message=FALSE,warning=FALSE>>=
	...
	newd <- expand.grid(Vessel = '58200', dum=0, year = unique(x$year), 
								depth = 44, set_lon = -155.0228, set_lat = 56.89265)
...
@
 ...
 \begin{table}[H]
   \centering
   \caption{Kodiak Southwest District catch summary for the 2009/10-2016/17 season for raw and standardized meat weight data. Data in this table are from the Kodiak Wiki*}
   \label{table:ksw2}
   \vspace{1.5ex}
  ...
  <<mwt_std_ksw,echo=FALSE,message=FALSE,warning=FALSE, results="asis">>=
x %>% 
	group_by(year) %>% 
...
@
 ...
 \begin{table}[H]
   \centering
   \caption{Kodiak Southwest District catch summary for the 2009/10-2016/17 season for raw and standardized round weight data. Data in this table are from the Kodiak Wiki*}
   \label{table:ksw3}
   \vspace{1.5ex}
   \begin{tabular}{lrrcrrc}
   \hline
  ...
  <<rwt_std_ksw,echo=FALSE,message=FALSE,warning=FALSE, results="asis">>=
x %>% 
	group_by(year) %>% 
...
<<map_ksw, fig.pos="H", fig.cap="Maps of District KSW fishing locations by fishing year.", echo=FALSE, warning=FALSE ,fig.height=8, cache=TRUE, dev='png', dpi=200>>=
f.district_map(x)
@

<<ksw_mwt_roll,echo=FALSE,fig.pos="H", fig.cap="Annual raw and standardized meat weight CPUE with all years mean, a rolling 5-year mean, and the past five years mean.", warning=FALSE ,fig.height=4, cache=FALSE, dev='png', dpi=300, warning=FALSE,message=FALSE>>=
x %>% 
	group_by(year) %>%
...
<<ksw_rwt_roll,echo=FALSE,fig.pos="H", fig.cap="Annual raw and standardized round weight CPUE with all years mean, a rolling 5-year mean, and the past five years mean.", warning=FALSE ,fig.height=4, cache=FALSE, dev='png', dpi=300, warning=FALSE,message=FALSE>>=
x %>% 
	group_by(year) %>%
	summarise(catch = sum(meat_weight, na.rm = T), round = sum(round_weight, na.rm = T),
				 hrs = sum(dredge_hrs), haul = length(meat_weight), cpue = catch/hrs, cpue2 = round/hrs)  %>% 
...
<<kswcpue, fig.pos="H", fig.cap="Density and number of tows by bed and year for District KSW meat weight cpue. The left pane shows the CPUE distribution for all of the hauls, the right pane shows the number of hauls for a given CPUE.",echo=FALSE, warning=FALSE ,fig.height=8.5, cache=FALSE, dev='png', dpi=300, message = FALSE>>=  
x %>% 
	filter(year>2011) %>% 
...
<<kswfit, fig.pos="H", fig.cap="Density and number of tows by bed and year for District KSW standardized meat weight cpue. The left pane shows the CPUE distribution for all of the hauls, the right pane shows the number of hauls for a given CPUE.",echo=FALSE, warning=FALSE ,fig.height=8.5, cache=FALSE, dev='png', dpi=300, message = FALSE>>=
x %>% 
	filter(year>2011) %>% 
...
<<kswcpue1, fig.pos="H", fig.cap="Density and number of tows by bed and year for District KSW round weight cpue. The left pane shows the CPUE distribution for all of the hauls, the right pane shows the number of hauls for a given CPUE.",echo=FALSE, warning=FALSE ,fig.height=8.5, cache=FALSE, dev='png', dpi=300, message = FALSE>>=  
x %>% 
	filter(year>2011) %>% 
...
<<kswfit1, fig.pos="H", fig.cap="Density and number of tows by bed and year for District KSW standardized round weight cpue. The left pane shows the CPUE distribution for all of the hauls, the right pane shows the number of hauls for a given CPUE.",echo=FALSE, warning=FALSE ,fig.height=8.5, cache=FALSE, dev='png', dpi=300, message = FALSE>>=
x %>% 
	filter(year>2011) %>% 
...
<<kswvessel, fig.pos="H", fig.cap="Annual vessel effect from meat weight cpue model fit, District KSW.",echo=FALSE, warning=FALSE ,fig.height=3.9, cache=FALSE,dev='png',dpi=300, message = FALSE>>=

      ggplot(x, aes(Year,fit, fill=Vessel))+geom_boxplot(width=.35)+ylab('Standardized CPUE')
@
   
<<kswvessel1, fig.pos="H", fig.cap="Annual vessel effect from round weight cpue model fit, District KSW.",echo=FALSE, warning=FALSE ,fig.height=3.9, cache=FALSE,dev='png',dpi=300, message = FALSE>>=
      #Model the CPUE
      #ggplot(ksw, aes(Year,fit, fill=Bed))+geom_boxplot()
      #ggplot(ksw, aes(Vessel,fit, fill=Bed))+geom_boxplot()
...
<<kswsh, fig.pos="H", fig.cap="Shell heights by bed and year for District KSW. The left pane shows the distribution of all of the samples, the right pane shows the number of samples by year (stacked bars).",echo=FALSE, warning=FALSE ,fig.height=8.5, cache=FALSE, dev='png', dpi=300, message = FALSE>>= 
...
<<kswsh1, fig.pos="H", fig.cap="Retained and discarded shell heights by bed for District KSW in 2016. The left pane shows the distribution for all of the samples, the right pane shows the number samples (stacked bars).",echo=FALSE, warning=FALSE ,fig.height=4, cache=FALSE, dev='png', dpi=300, message = FALSE>>=
ksw.sh %>% 
	filter(year==2016) %>% 
...
<<kswsh3, fig.pos="H", fig.cap="Shell heights by year, all beds combined, for District KSW. The left pane shows the distribution for all of the samples, the right pane shows the number of samples.",echo=FALSE, warning=FALSE ,fig.height=8.5, cache=FALSE,dev='png', dpi=300, message = FALSE>>=
...
<<kswage, fig.pos="H", fig.cap="Shell ages (annuli) by bed and year for District KSW. The left pane shows the  distribution for all of the annuli sampled, the right pane shows the number of samples.",echo=FALSE, warning=FALSE ,fig.height=8.5, cache=FALSE, dev='png', dpi=300, message = FALSE>>=   
 ksw %>% 
   	select(Haul_ID, Bed) %>% 
   	left_join(ages) %>% 
   	filter(year>2009) -> ksw.age

   a1 <- ggplot(ksw.age, aes(annuli, fill=Year, color=Year))+geom_density()+facet_grid(Bed~.)+
      scale_fill_manual(values=values, breaks=breaks, name='Year')+
      scale_color_manual(values=values,  breaks=breaks, name='Year')+
      guides(colour = guide_legend(override.aes = list(alpha = 1)))+ 
      theme(legend.key = element_blank(),legend.position = c(0.8, 0.39),legend.background = 
               element_rect(fill = "#ffffffaa", colour = NA),legend.key.height=unit(.6,'line'))
   a2 <- ggplot(ksw.age, aes(annuli, fill=Year, color=Year))+geom_histogram(binwidth=1)+facet_grid(Bed~.)+
      scale_fill_manual(values=values, breaks=breaks, name='Year')+
      scale_color_manual(values=values,  breaks=breaks, name='Year')+
      guides(colour = guide_legend(override.aes = list(alpha = 1)))+
      guides( fill=FALSE, color=FALSE)+ theme(legend.key = element_blank())
...
<<allageksw, fig.pos="H", fig.cap="Scallop ages for Kodiak Southwest District beds.",echo=FALSE, warning=FALSE ,fig.height=8.5, cache=FALSE, dev='png', dpi=300, message=FALSE>>=
ages %>% 
   filter(District=='KSW') %>% 
...
@
 ...
 \subsubsection{Scallop discards and crab/halibut bycatch}
  ...
  An estimated 8,017 Tanner Crab, 56 Pacific Halibut, and 65 red king crab were incidentally caught (Figure~\ref{fig:kswcrab}). An estimated 23,754 lb of scallops were discarded, 6.35\% of retained round weight. These numbers are on the high end of what has been observed since 2009.
  
<<kswcrab, fig.pos="H", fig.cap="Estimated Tanner crab, Pacific halibut, Opilio crab and red king crab bycatch for District KSW.",echo=FALSE, warning=FALSE ,fig.height=4, cache=FALSE, dev='png', dpi=300, eval=TRUE, message = FALSE>>=    
 a <- aggregate(Halibut_Count~year,data=subset(by, District=='KSW'), FUN=sum)
   ...
   a2 <- aggregate(Opilio_Count~year,data=subset(by, District=='KSW'), FUN=sum)
   a3 <- aggregate(King_Count~year,data=subset(by, District=='KSW'), FUN=sum)
...
<<kswdisc, fig.pos="H", fig.cap="Scallop estimated discard weight for District KSW.",echo=FALSE, warning=FALSE ,fig.height=4, cache=FALSE, dev='png', dpi=300, eval=TRUE, message = FALSE>>=   
    by %>% 
      filter(District=='KSW') %>% 
      group_by(year) %>% 
...
<<kswdisc1, fig.pos="H", fig.cap="Scallop estimated percent discards relative to retained round weight for District KSW.",echo=FALSE, warning=FALSE ,fig.height=4, cache=FALSE, dev='png', dpi=300, eval=TRUE, message = FALSE, png=TRUE, pdf=FALSE>>= 
   discards %>% 
